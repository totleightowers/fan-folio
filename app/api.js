...
const LOCAL = {
  ...
  home: () => {
     ...
     * works showed as twelve and the other eight looked lost rather than
     * folded away. See all is the way to the rest, and it can only be trusted
     * if the shelf admits there is a rest.
     */
    const shelf = (where, order, limit = 12) => ({
      ...
      total: sql(`SELECT count(*) AS n FROM works w LEFT JOIN reading r ON r.work_id = w.work_id
                  WHERE ${shown(where)}`)[0].n,
    });

    const totals = sql('SELECT count(*) AS works, COALESCE(sum(words),0) AS words FROM works w '
      + 'WHERE COALESCE(w.hidden, 0) = 0')[0];
    const read = sql(`
    ...
    const top = (kind, limit) => sql(
      `SELECT t.name, count(*) AS n FROM tags t JOIN works w ON w.work_id = t.work_id
       ...
       GROUP BY t.name ORDER BY n DESC LIMIT ${lim(limit, 12)}`,
      [kind]);
    ...
    return {
      ...
      shelves: [
        ...
        { key: 'short', title: 'One sitting',
          ...shelf(`w.complete = 1 AND w.words < 5000 AND ${STATES.unread}`, 'RANDOM()') },
      ...
      browse: {
        ...
        rating: sql(`SELECT rating AS name, count(*) AS n FROM works w
  ...
  surprise: () => ({
    work_id: sql(`SELECT w.work_id FROM works w LEFT JOIN reading r ON r.work_id = w.work_id
                  WHERE ${shown(STATES.unread)} ORDER BY RANDOM() LIMIT 1`)[0]?.work_id ?? null,
  }),
  ...
  work: (workId) => {
    ...
    const progress = one(
      'SELECT chapter, offset, chapters_read, opened_at, completed_before FROM reading WHERE work_id = ?', [workId]);
    return {
      ...work,
 ...
 */
export async function restartReading(workId) {
  const out = isNative ? JSON.parse(native.restartReading(String(workId)))
    : await fetch(`/api/restart?workId=${encodeURIComponent(workId)}`, { method: 'POST' }).then(r => r.json());
  if (out.error) throw new Error(out.error);
  return out;
}
...
export async function deleteWorks(workIds) {
  ...
  if (isNative) {
    const out = JSON.parse(native.deleteWorks(JSON.stringify(ids)));
    if (out.error) throw new Error(out.error);
    return out;
  }
  for (const id of ids) await deleteWork(id);
 ...
 *
 * A work with another author on it stays: blocking somebody should not cost
 * you a work you like that they happened to write half of.
 */
export async function blockAuthor(name) {
  if (isNative) {
    const out = JSON.parse(native.blockAuthor(String(name)));
    if (out.error) throw new Error(out.error);
    return out;
  }
  const res = await fetch(`/api/block?name=${encodeURIComponent(name)}`, { method: 'POST' });
  const out = await res.json();
  if (out.error) throw new Error(out.error);
  return out;
}
...
export async function unblockAuthor(name) {
  if (isNative) {
    const out = JSON.parse(native.unblockAuthor(String(name)));
    if (out.error) throw new Error(out.error);
    return out;
  }
  const res = await fetch(`/api/unblock?name=${encodeURIComponent(name)}`, { method: 'POST' });
  const out = await res.json();
  if (out.error) throw new Error(out.error);
  return out;
}
