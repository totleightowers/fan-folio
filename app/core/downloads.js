...
export function downloadStatus(jobs, { now = Date.now(), coolUntil = 0 } = {}) {
  ...
  const missing = jobs.reduce((n, j) => n + (Number(j.unfinished) || 0), 0);
  ...
  if (missing) return { title: 'Some works still need attention', detail: `${added} downloaded · ${missing} did not arrive. Retry the missing works below.`, active: 0, paused: 0 };
