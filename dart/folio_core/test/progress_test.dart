...
Database library() {
  ...
  db.execute("INSERT INTO works (work_id, title, authors) VALUES ('2', 'Bravo', '[\"bee\"]')");
  final ch = db.prepare('INSERT INTO chapters (work_id, number, html) VALUES (?,?,?)');
...
void main() {
  ...
  test('a chapter completes by being left', () {
    ...
    expect(db.select('SELECT chapters_read FROM reading').first['chapters_read'], 2,
  ...
  test('progress never goes backwards on its own', () {
    ...
    expect(row['chapter'], 2, reason: 'the place moves, because that is where you are');
    expect(row['chapters_read'], 4, reason: 'but what you have read stays read');
  });
  ...
  test('a work whose chapter count nobody knows can still be finished', () {
    ...
    expect(inState(db, 'finished'), contains('2'),
        reason: 'the chapters actually held answer it when the metadata cannot');
  });
  ...
  test('finishing can be taken back without losing the place', () {
    ...
    final row = db.select('SELECT chapter, offset, chapters_read FROM reading').first;
    expect(inState(db, 'reading'), contains('1'));
  ...
  test('opening a work is not the same as being somewhere in it', () async {
    ...
    final row = db.select('SELECT chapter, offset, opened_at FROM reading').first;
  ...
  group('where a chapter opens', () {
    test('where it was left off', () {
      expect(openingOffset(chapter: 3, savedChapter: 3, savedOffset: 1840), 1840);
    });
    ...
    test('a glance from a search result leaves no bookmark to return to', () {
      expect(openingOffset(chapter: 3, savedChapter: 3, savedOffset: 1840, transient: true), 0);
    });
    test('and nonsense opens at the beginning', () {
      expect(openingOffset(chapter: 1, savedChapter: 1, savedOffset: -5), 0);
      expect(openingOffset(chapter: 1, savedChapter: null, savedOffset: null), 0);
      expect(openingOffset(chapter: 1, savedChapter: 1, savedOffset: double.nan), 0);
    });
  });
}
...
class _Runner implements SqlRunner {
  ...
  final Database db;
  @override
  Future<List<Map<String, Object?>>> query(String sql, [List<Object?> a = const []]) async =>
